...
export function TechnicalProposalSummary({ proposal }: { proposal: TechnicalProposal }) {
  return (
    <section aria-labelledby="technical-proposal-title" className="space-y-4">
      ...
      <div>
        ...
        <div className="mt-3 grid min-w-0 gap-4 lg:grid-cols-2">
          {proposal.items.map((item) => <TechnicalProposalItemCard key={item.itemId} item={item} />)}
        </div>
      </div>
    </section>
  );
